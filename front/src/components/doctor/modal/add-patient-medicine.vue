					...
					<v-form ref="vForm" v-on:submit.prevent="submitMedicineList">
						<v-layout row wrap v-for="(medicine,index) in medicineList" :key="index">
							...
							<v-flex xs2 md3 class="pa-1">
								<v-text-field label="Days" :rules="[formRules.required,formRules.numberOnly]" v-model="medicine.medicinePieces" readonly></v-text-field>
							</v-flex>
				...
				<v-card-text>
					<v-layout row wrap>
						<v-flex>
							...
							<v-btn flat class="blue-grey darken-4 white--text" @click="openModal('showLabResults')">Go to Upload</v-btn>
						</v-flex>
					</v-layout>
				</v-card-text>
			</template>
	...
	export default {
		...
		methods : {
			openModal : function(modal){
				this.medicine = false;
				this.eventHub.$emit(modal, {'id': this.patient.id, 'status' : this.patient.status, 'category' : this.patient.category, 'data' : this.patient});
			},
			getMedicineCount : function(){
				switch (this.patient.category){
					case 'Cat I' :
						this.medicineCount = 52;
					break;
					case 'Cat II' :
						this.medicineCount = 102;
					break;
					case 'MDR' :
						this.medicineCount = 365;
					break;
				}
			},
			...
			addMedicineRow : function(){
				this.medicineList.push({
					medicineID : "",
					medicineDosage : "",
					medicinePieces : this.medicineCount,
				...
				.then(function(res){
					...
					if(res.data.status){
						for(let i in medicines){
							let dosage = (_this.patient.status=='New') ? '' : medicines[i].dosage;
							let pieces = (_this.patient.status=='New') ? _this.medicineCount : medicines[i].pieces;
							_this.medicineList.push({
								medicineID : medicines[i].id,
								medicineDosage : dosage,
								medicinePieces : pieces
							})
						}
					}
				});
			...
			submitMedicineList : function(){
				...
				n = date.getDay();
				if(n == 0 || n == 6){
					let returnval = { message : "Cannot set schedule in Saturdays and Sundays", icon : "error", color : "red"};
					...
					if(this.$refs.vForm.validate()){
						...
						.then(function(res){
							...
							_this.eventHub.$emit("updatePatientList",{ 'patientID' : _this.patient.id, 'status' : _this.patient.status, 'datestart' : new Date().toDateString().substr(4) });
						})
					}else{
